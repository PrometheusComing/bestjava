 ...
 *
 *  BIO模型（同步阻塞IO）：使用一个独立的Acceptor线程来监听客户端的连接（一个连接就是一个线程）
 *  弊端：每个客户端请求都需要一个线程去处理，无法满足高性能、高并发场景
 *
 *  伪异步IO：只是简单优化，底层仍旧使用bio，但是使用线程池去处理客户端请求，不会产生资源耗尽的情况
 ...
 *
 *  NIO模型（同步非阻塞IO）：使用IO多路复用(一个请求就是一个线程)，即客户端发送的连接请求都会注册到多路复用器上，多路复用器
 *  轮询到连接有I/O请求时才启动一个线程进行处理。用户进程也需要时不时的询问IO操作是否就绪，这就要求用户进程不停的去询问。
 *
